const MODEL_INFORMATION = {
	...
	model2: {
		...
		description:
			"This model was our next step to finding one that fit our needs for this project. This project utilized OpenCV with object detection. This object detection used centroid tracking by calculating the center of moving objects with their boundaries. This model worked well as it considered anything moving to be an object to track and determine its movement to see if the object is going in or out of the given building. This model was advantageous as its simplicity meant lower computation. This meant that a video stream could be used with this model as it could process a higher framerate (around 15+ fps) than that of the face detection model. Unfortunately, the simplicity of this model made its accuracy fall short in more extreme lighting conditions and angles and thus required the assumption that shelters would be higher contrast environments for this model to work its best. Another issue was the blending of two objects if they passed by each other which affected the accuracy by confusing the model. It was also discovered that using this model at a further distance allowed for better accuracy as the objects would then be in frame longer. ",
		videoSrc: "https://www.youtube.com/embed/NpEaa2P7qZI"
	},
	model3: {
		name: "TensorFlow Object Detection Model",
		description:
			"With the experience and insight gained from the two OpenCV detection models, this model began its development. This model used a lightweight TensorFlow model with training under the COCO dataset. The purpose of this model was to test a different approach of object detection using TensorFlow to generate bounding boxes for humans. The accuracy of this model in object detection was highly accurate and provided similar results to the OpenCV object detection model with a low demand for computation that resulted in a higher framerate (around 12 fps). Advantageous to the OpenCV object detection was this model’s ability to detect overlap of humans passing by each other which improved the count accuracy. This model however, used a centroid based approach to tracking and thus ran into similar issues as the OpenCV object detection with following object along the frame of the video stream. A potential improvement to this issue was to instead use Kalman filters which would help with tracking multiple objects and predict through estimates over time rather than independent (and potentially unreliable) centroid calculations. However, with time constraints and the later development of this model, this extension could not be implemented.",
		videoSrc: "https://www.youtube.com/embed/NpEaa2P7qZI"
	}
};
